...
while run:
    ...
    if event.type == pygame.MOUSEBUTTONDOWN and clicked:
        ...
        if clicked_card is not None:
            if not card.is_card_location_pressed(pygame.mouse.get_pos()):
    ...
    # Checks if there is a Clicked card and the flip the card (visual only)
    if clicked_card is not None:
